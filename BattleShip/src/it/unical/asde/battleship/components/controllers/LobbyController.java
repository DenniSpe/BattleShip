...
public class LobbyController {
	...
	@ResponseBody
	public DeferredResult<String> checkOwner(@RequestParam final String lobby_id) {// @RequestParam String lobbyID){
		...
		ForkJoinPool.commonPool().submit(() -> {
			...
			// String o = (!lobbyService.getLobby(ID).getOwner().equals(null) ? "owner" :
			// "notOwner");
	...
	public String createLobby(final Model model, @RequestParam final String lobby_name,
			@RequestParam final String lobby_owner, final HttpSession session) {
	...
	public String insideLobby(@RequestParam final String id, final Model model, HttpSession session, HttpServletResponse response) {
		...
		if (session.getAttribute("user") != null) {
			...
			if (mylobby.getWhoPlays() != null && !mylobby.getWhoPlays().isEmpty()) {
				return "redirect:/game?id=" + mylobby.getId();
			}
			model.addAttribute("lobbies", lobbyService.getLobbies());
			model.addAttribute("lobby", mylobby);
			model.addAttribute("currentLobbyID", mylobby.getId());
	...
	public String joinLobby(final Model model, final HttpSession session, @RequestParam final String lobby_id) {
		...
		if (session.getAttribute("user") != null) {
			...
			if (myLobby.getChallenger() == null && !user.getUsername().equalsIgnoreCase(myLobby.getOwner())) {
				...
				System.out.println(
						"======================JOIN LOBBY CHALLENGER AGGIUNTO ========" + myLobby.getChallenger());
	...
	@PostMapping("/waiting")
	@ResponseBody
	public DeferredResult<String> waiting(final Model model, final HttpSession session,
			@RequestParam final String lobbyID) throws InterruptedException {
		System.out
				.println("===================================== INIZIO WAITING =====================================");
